...
func (a *App) Use(middleware func(*fiber.Ctx) error) *App {
	if isLoggerMiddleware(middleware) {
		panic(fiber.DefaultColors.Red + "Do not set the logger middleware manually. Instead use DisableRequestLogging in the blueRPC config that you are using" + fiber.DefaultColors.Reset)
	}
	a.fiberApp.Use(middleware)
	return a
}
...
// same as the Listen method in fiber. Default port is 443
func (a *App) Listen(port ...string) error {
	...
	return a.fiberApp.Listen(actualPort)
}
